...
public class Module_BaseV1 : Module
{
   Vector3 Node1Offset = new Vector3(0.0f, 0.3f, 1.9f);
    Vector3 Node2Offset = new Vector3(0.0f, 0.3f, -1.9f);
    Vector3 Node3Offset = new Vector3(1.9f, 0.3f, 0.0f);
    Vector3 Node4Offset = new Vector3(-1.9f, 0.3f, 0.0f);
    float myFatAss = 1.90f;

    public Module_BaseV1(Vector3 pos, GameObject Node, GameObject me) {
        nNodes = 4;
    ...
    public override Vector3 connect(Vector3 modulePos, Vector3 nodeOffset, char axis, float FatOffset) {
        ...
        if(axis == 'x') {
            if(nodeOffset.x > 0) {
                return modulePos + nodeOffset + new Vector3(1.9f , -0.3f, 0.0f) ;
            }else if( nodeOffset.x < 0) {
                return modulePos + nodeOffset + new Vector3(-1.9f  , -0.3f, 0.0f) ;
            }
        }else if(axis == 'z') {
            if(nodeOffset.z > 0) {
                return modulePos + nodeOffset + new Vector3(0.0f, -0.3f, 1.9f + FatOffset) ;
